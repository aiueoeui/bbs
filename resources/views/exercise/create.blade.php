      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
          <form action="{{ route('exercise.store') }}" method="POST">
            @csrf
            <div>
              <label for="exercise_name">{{ __('運動種目') }}</label>
              <textarea name="exercise_name" id="exercise_name" cols="30" rows="2" class="w-full rounded-lg border-2 bg-gray-100 @error('title') border-red-500 @enderror"></textarea>
