...
function anglereslt_2() {
    ...
    if (flag_2 == true) {
        ...
        text(target_angle_l2 + leftflexiontext_02 + "°" + anglestatus2, 1, 30);//北山：anglestatus追加
        text(target_angle_r2 + rightflexiontext_02 + "°" + anglestatus2, 1, 90);//北山：anglestatus追加
    } else {
        text(target_angle_l2 + leftflexiontext_02 + "°" + anglestatus2, 1, 30);//北山：anglestatus追加
        text(target_angle_r2 + rightflexiontext_02 + "°" + anglestatus2, 1, 90);//北山：anglestatus追加
    }
...
function countnumber(){//小林行追加
    uttr.text = (conditions_count);
...
function conditions() {
    ...
    if (HipAbovenose == true) {//肩角度チェック
        if ((rightflexiontext_01 > 120 && rightflexiontext_01 < 140) && (leftflexiontext_01 > 140 && leftflexiontext_01 < 160)) {
            ...
            anglestatus ="もう少し体をよこにのばそう";
            BADANGLESTATUS += 1;
            flag_1 = false;
            uttr.text = "もう少しからだを横に伸ばそう";//小林行追加
        ...
        if (flag_1 == true) {//肘角度チェック
            if ((leftflexiontext_02 >= 150 && rightflexiontext_02 >= 130)) {
                ...
                anglestatus2 ="もう少しうでをのばそう";
                BADANGLESTATUS += 1;
                flag_2 = false;
                uttr.text = "もう少しうでを伸ばそう";//小林行追加
                speechSynthesis.speak(uttr);//小林行追加
            }
        }
...
setInterval(function () {
    if (timerflag == true) {
        if (conditions_count > 0) {
            conditions_count -= 1;
            setTimeout(countnumber,1000)//小林行追加
